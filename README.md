...
Hi. I'm a starter theme called `gp-strategies`. I'm a theme meant for hacking so don't use me as a Parent Theme. Instead, try turning me into the next, most awesome, WordPress theme out there. That's what I'm here for.
...
`gp-strategies` requires the following dependencies:
...
1. Search for `'gp-strategies'` (inside single quotations) to capture the text domain and replace with: `'megatherium-is-awesome'`.
2. Search for `gp_strategies_` to capture all the functions names and replace with: `megatherium_is_awesome_`.
3. Search for `Text Domain: gp-strategies` in `style.css` and replace with: `Text Domain: megatherium-is-awesome`.
4. Search for <code>&nbsp;Gp_Strategies</code> (with a space before it) to capture DocBlocks and replace with: <code>&nbsp;Megatherium_is_Awesome</code>.
5. Search for `gp-strategies-` to capture prefixed handles and replace with: `megatherium-is-awesome-`.
6. Search for `GP_STRATEGIES_` (in uppercase) to capture constants and replace with: `MEGATHERIUM_IS_AWESOME_`.

Then, update the stylesheet header in `style.css`, the links in `footer.php` with your own information and rename `gp-strategies.pot` from `languages` folder to use the theme's slug. Next, update or delete this readme.

### Setup

To start using all the tools that come with `gp-strategies`  you need to install the necessary Node.js and Composer dependencies :
...
`gp-strategies` comes packed with CLI commands tailored for WordPress theme development :

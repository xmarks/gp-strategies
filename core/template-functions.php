...
if ( ! function_exists( 'gp_strategies_pingback_header' ) ) {
	/**
	 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
	 */
	function gp_strategies_pingback_header() {
		if ( is_singular() && pings_open() ) {
			printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
		}
	}
}
add_action( 'wp_head', 'gp_strategies_pingback_header' );

if ( ! function_exists( 'gp_strategies_allowed_html_head_tags' ) ) {
	/**
	 * Allowed tags for custom HTML in <head>.
	 *
	 * @return array
	 */
	function gp_strategies_allowed_html_head_tags(): array {
		return array(
			'script'   => array(
				'type'  => true,
				'src'   => true,
...
if ( ! function_exists( 'gp_strategies_wp_kses_header_code' ) ) {
	/**
	 ...
	 * @return string
	 */
	function gp_strategies_wp_kses_header_code( string $input ): string {
		return wp_kses( $input, gp_strategies_allowed_html_head_tags() );
	}
}
...
if ( ! function_exists( 'gp_strategies_nav_menu_add_screen_option_columns' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gp_strategies_nav_menu_add_screen_option_columns( $cols ) {
		$cols['unlink'] = __( 'Unlink', 'gp-strategies' );
		$cols['image']  = __( 'Image', 'gp-strategies' );
		return $cols;
	}
}
add_filter( 'manage_nav-menus_columns', 'gp_strategies_nav_menu_add_screen_option_columns', 20 );

if ( ! function_exists( 'gp_strategies_nav_menu_custom_fields' ) ) {
	/**
	 ...
	 * @return void
	 */
	function gp_strategies_nav_menu_custom_fields( $item_id ) {
		...
		<p class="field-unlink description description-thin"><!-- class matches Screen‑Option slug -->
			<label for="edit-menu-item-unlink-<?php echo esc_attr( $item_id ); ?>">
				...
				<?php esc_html_e( 'Unlink', 'gp-strategies' ); ?>
			</label>
		</p>
		<?php
		...
		<p class="field-image description description-wide">
			<label>
				<?php esc_html_e( 'Image', 'gp-strategies' ); ?>
			</label><br>
			...
			<span class="menu-image-wrapper <?php echo $has_image ? 'has-image' : ''; ?>">
				...
				<button type="button"
					...
					echo esc_html(
						$has_image
							? __( 'Change', 'gp-strategies' )
							: __( 'Set image', 'gp-strategies' )
					);
					?>
				</button>
				...
				<button type="button"
					...
					<?php esc_html_e( 'Remove', 'gp-strategies' ); ?>
				</button>
			</span>
		</p>
		<?php
...
add_action( 'wp_nav_menu_item_custom_fields', 'gp_strategies_nav_menu_custom_fields', 10, 2 );

if ( ! function_exists( 'gp_strategies_nav_menu_save_custom_fields' ) ) {
	/**
	 ...
	 * @return void
	 */
	function gp_strategies_nav_menu_save_custom_fields( $menu_id, $menu_item_db_id ) {
		/*
...
add_action( 'wp_update_nav_menu_item', 'gp_strategies_nav_menu_save_custom_fields', 10, 2 );
